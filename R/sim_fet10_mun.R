#' Scrapes SIM's ICD-10 fetal mortality data from cities
#'
...
#' fetal mortality data focused on brazilian cities.
#'
#' @usage sim_fet10_mun(linha = "Município", coluna = "Não ativa",
...
#' @param obito_relacao_parto "all" or a character vector with period of fetal mortality in relation to childbirth (written in the same way) or the number corresponding to the order of the option in the online layout to filter the data. Defaults to "all".
...
#' sim_fet10_mun(municipio = 500270)
#' }
#'
#' @keywords SIM datasus fetal mortality
#' @importFrom magrittr %>%
#' @importFrom utils head
#' @export

sim_fet10_mun <- function(linha = "Munic\u00edpio", coluna = "N\u00e3o ativa", conteudo = 1, periodo = "last", municipio = "all", capital = "all",
  ...
  page <- xml2::read_html("http://tabnet.datasus.gov.br/cgi/deftohtm.exe?sim/cnv/fet10br.def")
  ...
  site <- httr::POST(url = "http://tabnet.datasus.gov.br/cgi/tabcgi.exe?sim/cnv/fet10br.def",
                     body = form_data)
